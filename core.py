...
_VOICES = [
    ...
    "대학생 / 반말 / 툭툭 던지는 구어체 / 'ㄹㅇ' 'ㄱㄱ' 같은 건 안 쓰되 리듬은 가볍게",
...
def diversity_block():
    """매 호출마다 골격·화자·훅을 랜덤 지정 → 같은 틀 복제를 원천 차단."""
    sk = _random.choice(_SKELETONS)
    vc = _random.choice(_VOICES)
    hk = _random.choice(_HOOKS)
    return (
        "■ 이번 글의 지정 설정 (반드시 이대로, 다른 예시 틀을 복사하지 마라):\n"
        f"  · 본글 골격: {sk}\n"
        f"  · 화자 페르소나: {vc}\n"
        f"  · 훅 전략: {hk}\n"
        "  · 위 골격·화자·훅은 이 글만의 것이다. 예시들은 '결'만 참고하고 문장은 절대 베끼지 마라.\n"
        "  · 답글 5개도 서로 다른 리듬으로: 하나는 아주 짧게(한 줄), 하나는 길게, "
        "하나는 자문자답, 하나는 숫자 포함. 다섯 개가 비슷하면 실패다.\n"
    )
