    ...
    if channel == "threads":
        ...
        POOLS = {
            "polite": {
                ...
                "r3": ["지금은 잘 쓰고 있습니다", "한동안은 이걸로 갈 것 같아요", "생각보다 손이 자주 가네요"],
                "r4": ["물론 사람마다 다를 수 있어요", "저한테 맞았던 거라 정답은 아니고요", "완벽한 건 아니에요ㅎㅎ", "아직 몇 번 안 써봐서 더 봐야겠지만요"],
            ...
            "expert": {
                ...
                "r4": ["다만 조건에 따라 달라질 수 있습니다", "모든 경우에 해당하진 않습니다"],
            ...
            "casual": {
                ...
                "r4": ["뭐 취향 차이일 수도 있고", "나만 그런 걸 수도 있음"],
            ...
            "friendly": {
                ...
                "r3": ["한동안은 이걸로 갈 것 같아요", "생각보다 손이 자주 가네요", "지금은 그냥 두고 씁니다"],
                "r4": ["물론 사람마다 다를 수 있어요", "저한테 맞았던 거라 정답은 아니고요", "완벽한 건 아니에요ㅎㅎ", "아직 더 써봐야 알겠지만요"],
        ...
        # ★7분할: 본글+답글1~6. 링크는 답글5·6에만.
        _r4 = R(P.get("r4") or P["r3"])
        _tries = 0
        while _r4 == R(P["r3"]) and _tries < 4:      # 답글3과 겹치면 다시
            _r4 = R(P.get("r4") or P["r3"]); _tries += 1
        parts = [R(P["posts"]), R(P["r1"]), R(P["r2"]), R(P["r3"]), _r4, r4, r5]
        return "\n===THREAD===\n".join(parts)
...
_TOPIC_PRODUCTS = {
    ...
    "빨래 건조|빨래건조|빨래|세탁|쉰내|섬유유연|옷 냄새": ["세탁조 클리너", "세탁세제", "섬유유연제", "빨래 건조대", "탈취제", "실내 건조대"],
    "숙면|불면|잠자리|수면|베개|매트리스|잠못|잠들": ["냉감 이불", "경추 베개", "쿨매트", "메모리폼 토퍼", "수면 안대", "바디필로우"],
    "모기|벌레|해충|방충": ["모기 퇴치기", "방충망", "모기향", "해충 트랩", "전기 모기채", "포충기"],
    "세균|위생|살균|소독|청결": ["살균기", "손소독제", "행주", "식기 건조대", "도마 살균", "청소 티슈"],
    "자동차|차량|운전|주차|카시트|블랙박스": ["차량용 햇빛가리개", "차량용 선풍기", "블랙박스", "차량 방향제", "핸드폰 거치대", "썬팅 필름"],
    "반려|강아지|고양이|펫|댕댕|냥": ["펫 쿨매트", "펫 급수기", "쿨 밴드", "탈취 스프레이", "빗", "자동 급식기"],
    "아기|육아|유아|신생아|기저귀": ["기저귀", "물티슈", "젖병 소독기", "베이비 모니터", "아기 쿨매트", "수유등"],
    "피부|햇빛|자외선|피부건조|보습|각질|트러블": ["선크림", "수분 크림", "미스트", "핸드크림", "보습 마스크팩", "바디로션"],
    "운동|다이어트|홈트|헬스": ["요가 매트", "폼롤러", "아령", "저항 밴드", "단백질 쉐이크", "스마트 워치"],
    "청소|정리|수납|먼지": ["무선 청소기", "정리 수납함", "밀대 걸레", "먼지 떨이", "수납 정리대", "청소포"],
    "식기 건조|식기건조|주방|설거지|요리|식기|그릇|수세미": ["주방세제", "수세미", "실리콘 장갑", "식기 건조대", "밀폐용기", "도마"],
...
def _rule_products_for(title):
    """제목에 맞는 카테고리 상품을 돌려준다.
    ★첫 매칭이 아니라 '가장 구체적으로 맞은' 카테고리를 고른다.
    (예전엔 '식기 건조대'의 '건조'가 피부 카테고리에 먼저 걸려 선크림이 나왔다)"""
    best, best_score = [], 0
    for pat, prods in _TOPIC_PRODUCTS.items():
        ...
        if not hits:
            continue
        # 점수: 맞은 단어들의 길이 제곱 합 (긴 단어일수록 구체적) + 개수 보너스
        score = sum(len(w) ** 2 for w in hits) + (len(hits) - 1) * 2
        if score > best_score:
            best, best_score = prods, score
    return best
...
def claude_write_thread(api_key, product_name, deeplink, tone="friendly", price=None, extra="", fast=False, deeplink2=""):
    ...
    for _ in range(budget):
        ...
        if fast:
            fails = critical_only(fails)   # ★속도: 사소한 건 넘어간다
        if not fails:
            break
...
# ── ★식품·먹거리 골격: '저녁 딜레마' ──
SKELETON_DINNER = """[골격: 저녁 딜레마]
① 본글 — "오늘 뭐 먹지"가 시작되는 실제 장면 하나. 시간·상황·피로가 보이게.
   예) 여덟시 넘어서 냉장고 열어보고 그냥 닫았다.
② 배달을 시키려다 멈춘 이유. 돈·건강·귀찮음 중 하나만 골라 솔직하게.
③ 결국 뭘 했는지 구체적 행동. 재료 하나, 불 세기, 시간 같은 것.
④ 의외의 발견 — 생각보다 쉬웠거나, 맛이 예상과 달랐거나.
⑤ ★실패담 하나. 태웠다·안 익었다·짰다. 이게 없으면 광고로 읽힌다.
⑥ 그래서 지금은 이렇게 한다 — 반복 가능한 방식으로.
⑦ 링크 + 고지문구.

금지: "강추", "인생템", 별점, 가격 자랑, 원산지 나열.
톤: 배고픈 사람이 배고픈 사람에게. 요리 잘하는 척 금지."""

# ── ★검증 골격: '목격담' (릴스 상위계정 19편 분석, 2026-08-17) ──
#   전부 "내가 샀다"가 아니라 "남이 쓰는 걸 봤다"로 시작한다.
#   내가 사면 광고, 남이 쓰는 걸 목격하면 정보가 된다.
SKELETON_WITNESS = """[골격: 목격담]
① 본글 — ★반드시 제3자가 쓰는 장면을 목격한 것으로 시작한다. 내가 산 얘기 금지.
   예) 회사 동료차에 탔다가 / 인테리어 하는 친구가 / 회의 중에 꺼내길래
   화자는 관찰자다. 아직 제품 이름을 말하지 않는다.
② 오인 — 처음엔 다른 물건인 줄 알았다고 쓴다. 정체를 늦게 밝힌다.
   예) 예쁜 스피커인 줄 알았는데 / 지갑인 줄 알고 / 이게 가습기였습니다
③ 놀람 한 줄 — "진짜 깜짝 놀랐다", "왜 이걸 이제 알았을까".
④ 그래서 나도 써봤다 — 여기서 처음 화자가 사용자가 된다.
⑤ 구체 사실 하나 — 숫자·시간·감각. 9cm로 접힌다, 3분이면 냄새가 빠진다.
⑥ ★단정 회피 — "사람마다 다를 수 있다", "저한테 맞았던 거라".
⑦ 링크 + 고지.

★가족을 동기로 쓰면 더 강하다: 딸아이 머리 말려주다가 / 아내가 잔소리해서.
   나를 위한 소비보다 정당화가 쉽다.

금지: "내돈내산", "강추", 별점, 스펙 나열, 첫 줄에 제품명."""
...
# 치명적(반드시 고쳐야) vs 사소한(그냥 둬도 되는) 위반 구분
_CRITICAL_MARKS = ("깨진 문자", "외국 문자", "지어", "인증", "플레이스홀더",
                   "빈 감성", "제품명", "무슨 물건", "숫자가 안 맞", "반복된다")


def critical_only(fails):
    """빠른 모드에서는 치명적인 것만 고친다. 문장 리듬 같은 건 그냥 둔다."""
    return [f for f in fails if any(m in f for m in _CRITICAL_MARKS)]
