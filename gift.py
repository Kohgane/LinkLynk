...
def _extra_lines(taste):
    """추가 질문 축: 관계 온도 / 실용·감성 / 금기 — 칩 문구 감지."""
    t = taste or ""
    L = []
    if "가벼운 사이" in t:
        L.append("★관계 온도=가벼움: 부담 없는 캐주얼 선물. 지나치게 사적·로맨틱한 품목"
                 "(향수, 커플템, 신체 사이즈를 알아야 하는 것) 금지.")
    if "각별한 사이" in t:
        L.append("★관계 온도=각별함: 의미와 이야기가 있어 오래 남는 물건 위주 — 소모품보다 간직하는 것.")
    if "실용 위주" in t:
        L.append("★실용파: 매일 실제로 쓰는 물건만. 장식·오브제·감상용 금지.")
    if "감성 위주" in t:
        L.append("★감성파: 쓸모보다 무드·아름다움·기분. 밋밋한 생활 실용품 금지.")
    for k, ban in (("술 제외", "주류·술잔·바 용품"), ("향 제외", "향수·인센스·캔들·디퓨저"),
                   ("먹거리 제외", "식품·차·커피·간식"), ("전자기기 제외", "전자·디지털 기기"),
                   ("피부접촉 제외", "화장품·바디용품·의류·액세서리")):
        if k in t:
            L.append(f"★금기: {ban} 계열 절대 금지 — 한 픽도 내지 마라.")
    return ("\n".join(L) + "\n") if L else ""
...
def recommend(api_key, who, budget, taste, exclude=None):
    ...
    user = (
        ...
        + ("★각 방향은 서로 다른 카테고리 패밀리에서 하나씩만: 문구·필기 / 홈카페·차 / "
           ...
           "조명·오브제 / 데스크셋업. 이전 추천이 속한 패밀리는 제외하고 남은 것에서 고른다. 단 angle에는 패밀리명을 그대로 쓰지 말고 감각적인 이름을 지어라(X: '바디·그루밍' / O: '향으로 남는 손끝').\n"
           if reroll else "")
        +
