# -*- coding: utf-8 -*-
"""가라고 세계화 — OSM 화장실 수집기 v2
   toilets_kr.json과 동일 스키마(+cc, oh)로 도시별 파일 생성.
   사용: python3 osm_toilet_collect_v2.py [도시키 ...]
        python3 osm_toilet_collect_v2.py --list
"""
...
def norm(el, cc):
    ...
    fee = 1 if (t.get("fee") or "").lower() in ("yes", "1", "true") else 0
...
def collect(key):
    ...
    for i, (a, b, c, d) in enumerate(tl, 1):
        ...
        if r is None:
            ...
            print("    [%d/%d] 실패 (누적 %d) %ds 쿨다운" % (i, len(tl), len(fails), COOLDOWN))
            sys.stdout.flush()
            time.sleep(COOLDOWN)
            continue
        got = absorb(r, cc, seen, rows)
    ...
    for rnd in range(3):
        ...
        for (a, b, c, d) in retry:
            ...
            json.dump(rows, open(path, "w", encoding="utf-8"), ensure_ascii=False)
            time.sleep(PAUSE)
    meta = {"key": key, "label": label, "cc": cc, "tz": tz,
            "bbox": [s, w, n, e], "count": len(rows),
            "failed_tiles": len(fails), "built": time.strftime("%Y-%m-%d")}
    json.dump(meta, open(os.path.join(OUT, "%s.meta.json" % key), "w", encoding="utf-8"),
              ensure_ascii=False, indent=1)
    print("  => %s.json  %d건 (미완 타일 %d)" % (key, len(rows), len(fails)))
    sys.stdout.flush()
    return len(rows)
