...
def dwaeji_reco_api():
    ...
    from core import llm_chat, _parse_json_out, scrub_garbled
    sysp = (
        "너는 전국 맛집을 발로 뛰어 아는 한국 미식 큐레이터 '돼지레이다'다. "
        "먹는 것, 그것은 삶의 행복 — 이 마음으로 추천한다.\n"
        "★배제 기준은 '체인이냐'가 아니라 '맛이냐'다. 프랜차이즈·백화점 식당가도 "
        "요리하는 사람 따라 맛이 다르니 배제하지 마라 — 단, 추천한다면 '어느 지점이 "
        "왜 다른지'까지 짚어라(예: 'OO 본점만 직화, 지점은 철판'). 걸러야 할 건 "
        "광고·바이럴로만 부풀려진 집과 뜨내기 호객집뿐이다.\n"
        "★해상도가 생명 — 뭉뚱그리지 말고 세세히·촘촘히: 메뉴는 '뭘 어떻게 시켜야 "
        "하는지'(곱빼기/토렴/소금 먼저 같은 디테일), 맛은 '왜 맛있는지'(육수 내는 법, "
        "직접 뽑는 면, 숙성 일수, 불맛)까지.\n"
        "★솔직함이 신뢰다 — 식사일기처럼 써라: 단점을 숨기지 마라(협소한 공간, 웨이팅, "
        "불친절, 현금만, 주차 지옥). 장점만 나열하는 광고 문체 금지.\n"
        "★정직성: 실존을 확신하는 가게만 이름을 대라. 확신이 약하면 name에 "
        "'(방문 전 확인)'을 붙이거나 실존하는 '류'로 안내(예: '부평깡통시장 유부주머니 "
        "골목'). 없는 가게를 지어내느니 4곳만 내라.\n"
        "출력 JSON: {\"picks\":[{\"name\":\"상호(지점까지)\",\"area\":\"동네·랜드마크 기준 위치\","
        "\"dish\":\"주문 공식 — 메뉴+방법(예: 수육백반에 막국수 반반, 새우젓 먼저)\","
        "\"why\":\"맛의 근거 한두 문장 — 조리·재료·식감 디테일\","
        "\"cons\":\"솔직한 단점 한 줄(없으면 '딱히 없음')\","
        "\"diary\":\"식사일기풍 한 줄 감상(예: '국물 한 술에 어깨가 풀렸다. 재방문 100번.')\","
        "\"tip\":\"타이밍·웨이팅 회피 팁 한 줄\",\"q\":\"네이버지도 검색어(지역+상호)\"}x5]}"
    )
    ...
    for p in picks:
        out.append({"name": scrub_garbled(str(p.get("name") or ""))[:40],
                    ...
                    "dish": scrub_garbled(str(p.get("dish") or ""))[:70],
                    "why": scrub_garbled(str(p.get("why") or ""))[:160],
                    "cons": scrub_garbled(str(p.get("cons") or ""))[:80],
                    "diary": scrub_garbled(str(p.get("diary") or ""))[:90],
...
def gift_config_api():
    ...
    resp = jsonify({"ok": True,
                    ...
                    "questions": [
                        {"id": "rel", "step": "받는 분", "t": "누구에게 주나요?", "type": "single",
                         "opts": ["부모님", "연인", "친구", "직장 동료·상사", "형제·자매", "아이·조카", "선생님·은사", "나 자신"]},
                        {"id": "a", "step": "받는 분", "t": "나이대", "type": "single",
                         "opts": ["10", "20", "30", "40", "50", "60", ""],
                         "labels": ["10대", "20대", "30대", "40대", "50대", "60대+", "모름"]},
                        {"id": "g", "step": "받는 분", "t": "성별", "type": "single",
                         "opts": ["여", "남", ""], "labels": ["여성", "남성", "상관없음"]},
                        {"id": "close", "step": "받는 분", "t": "얼마나 가까운 사이?", "type": "single",
                         "opts": ["아주 가까움", "편한 사이", "예의 갖출 사이", "잘 모르는 사이"]},
                        {"id": "occ", "step": "상황", "t": "어떤 자리인가요?", "type": "single",
                         "opts": ["생일", "기념일", "집들이", "감사 인사", "승진·취업", "결혼·출산", "명절", "그냥, 문득"]},
                        {"id": "budget", "step": "상황", "t": "예산", "type": "single",
                         "opts": ["1만원 이하", "1~3만원", "3~5만원", "5~10만원", "10~20만원", "20만원 이상"]},
                        {"id": "when", "step": "상황", "t": "언제 필요한가요?", "type": "single",
                         "opts": ["오늘·내일", "이번 주", "여유 있음"]},
                        {"id": "vibe", "step": "취향", "t": "받는 분은 어떤 사람? (복수 선택)", "type": "multi", "max": 3,
                         "opts": ["실용파 — 쓸모가 최고", "감성파 — 예쁜 게 최고", "미니멀 — 물건 적은 편",
                                  "맥시멀 — 모으는 재미", "트렌디 — 요즘 것", "클래식 — 오래 가는 것",
                                  "장인 — 만듦새를 봄", "수집가 — 시리즈로 모음"]},
                        {"id": "cats", "step": "취향", "t": "끌릴 만한 분야 (복수 선택)", "type": "multi", "max": 7,
                         "opts": ["홈카페·차", "향·캔들", "주방·리빙", "문구·데스크", "술·잔", "패션잡화",
                                  "바디·그루밍", "오디오·아날로그", "아웃도어·운동", "게임·취미",
                                  "독서·문학", "가드닝·식물", "여행·트래블", "베이킹·디저트",
                                  "반려동물", "아트·공예", "테크·가젯", "홈트·요가"]},
                        {"id": "nocats", "step": "취향", "t": "이건 피할까요 (복수 선택)", "type": "multi", "max": 7,
                         "opts": ["먹는 것", "향 나는 것", "몸에 닿는 것", "전자기기", "장식품",
                                  "술 관련", "옷·사이즈 타는 것", "부피 큰 것", "없음 — 다 좋아요"]},
                        {"id": "who", "step": "취향", "t": "이 사람, 한마디로", "type": "text",
                         "ph": "예: 땀이 많아요 / 캠핑 좋아함 / 자취 시작 / 손이 작아요",
                         "hint": "한 줄이면 충분해요. 이게 추천을 가장 크게 바꿉니다."}
                    ]})
    resp.headers["Access-Control-Allow-Origin"] = "*"
    return resp
...
def assetlinks():
    ...
    _mj_default = (
        "91:59:BE:7E:D8:1F:59:8F:3E:9E:45:E7:88:4C:D7:39:25:57:82:95:68:42:30:F2:C7:9B:05:F3:EF:23:18:E5,"  # Play 앱 서명 키
        "9D:D6:0B:2F:E9:52:EA:97:96:CB:2D:1F:0E:35:FF:46:7E:F4:F3:CF:9D:64:EF:75:E9:0E:01:1F:DE:D0:88:51,"  # 업로드 키(콘솔 표기)
        "8E:6F:90:36:2D:D4:5B:BB:9B:09:54:60:53:4D:30:F2:38:42:5F:7B:E2:05:48:C8:E0:FB:69:E9:00:AD:7E:B9")  # 빌드 키(자체 키스토어)
...
def ember_api(action):
    ...
    if action == "botset":
        p = ember.bot_set(code, str(d.get("av") or ""), str(d.get("mood") or ""),
                          str(d.get("vibe") or ""), str(d.get("name") or ""))
...
@app.route("/fly/v2/<path:fname>")
def fly_v2_assets(fname):
    return send_from_directory("static/fly/v2", fname)
