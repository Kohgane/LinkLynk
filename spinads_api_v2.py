...
def spinads_brief():
    ...
    with _db() as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("""select g.d::text, coalesce(v.sessions,0) sessions, coalesce(v.clicks,0) clicks,
                              coalesce(v.publisher_earn_krw,0) publisher_earn_krw
                           from generate_series(current_date-6, current_date, '1 day') g(d)
                           left join dw.spinads_daily v on v.d = g.d::date order by g.d desc""")
    ...
    actions = []
    if pending["n"] > 0:
        actions.append(f"광고주 신청 {pending['n']}건 대기(예산 합계 ₩{pending['budget']:,}) — [[런북 광고주 활성화]]. 이게 오늘 가장 빠른 돈")
    if pubs <= 1:
        actions.append("퍼블리셔 아직 1명(본인) — [[런북 쓰레드 발사]] / [[런북 크몽 등록]]. 네트워크는 두 번째 설치자부터")
    if s7 > 0 and c7 == 0:
        actions.append(f"7일 세션 {s7}건인데 클릭 0 — [[런북 클릭 0 대응]]")
    if c7 > 0:
        actions.append(f"7일 클릭 {c7}건 — 쿠파스 대시보드 subId=spinads 수익 확인")
    if sugg > 0:
        actions.append(f"문구 공모 {sugg}건 대기 — 채택은 공짜 콘텐츠이자 첫 커뮤니티")
    if not actions:
        actions.append("대기 업무 없음 — 오늘은 파는 날: [[런북 후원 DM]] 1통")
...
def spinads_health():
    return jsonify(ok=True, service="spinads", version="v4.4")
